document.addEventListener("DOMContentLoaded", () => {
  ...
  const modeloRadios = document.querySelectorAll("input[name='modeloSelloAutomaticoEscolar.id']");
  const cantidadInput = document.getElementById("cantidad");
  const totalInput = document.getElementById("total");
  const abonadoInput = document.getElementById("abonado");
  const restaInput = document.getElementById("resta");
  const adicionalDisenioCheckbox = document.getElementById("sello-automatico-escolar-con-adicional-disenio");
  ...
  function buscarPrecio() {
    ...
    const params = new URLSearchParams({
      modeloSelloAutomaticoEscolarId: modeloId
    });

    fetch(`/api/plantilla-sello-automatico-escolar/precio?${params.toString()}`)
